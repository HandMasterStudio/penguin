...
GLFWwindow *WindowManager::getWindowID() const { return window; }

int WindowManager::getWindowSizeH() const { return h; }

int WindowManager::getWindowSizeW() const { return w; }

bool WindowManager::getIsPause() const { return isPause; }

const char *WindowManager::getTitleString() const { return title; }
...
void WindowManager::statePause() {
  ...
  if (isPause) {
    glfwSetInputMode(windowManager.getWindowID(), GLFW_CURSOR,
                     GLFW_CURSOR_NORMAL);
  } else {
    glfwSetInputMode(windowManager.getWindowID(), GLFW_CURSOR,
